...
public class DidlDaoSupport {
    ...
    public String translateSearchCriteria(final String search,
            final List<Object> parameters) {
        // upnp:class derivedfrom "object.item.imageItem.photo" and (dc:date >=
        // "2001-10-01" and dc:date <= "2001-10-31")
        // dc:creator = "Sting"
        final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

        final String[] words = search.replaceAll("\\(", "\\( ")
                .replaceAll("\\)", " \\)").split(" ");
                            ...
                            if (valid) {
                                ...
                                if (isDate) {
                                    ...
                                    if (isDerivedClass) {
                                        ...
                                        stmt.append("(");
                                        for (final ClassNameWcType ct : ClassNameWcType
                                                .values()) {
                                            if (ct.value()
                                                    .toLowerCase()
                                                    .startsWith(
        ...
        logger.info("requested search [" + search + "] was transformed to ["
                + result + "]");
        return result;
    }
    ...
    /**
     * has to translate the didl sortcriteria into hql ' order by didl.title' or
     * something
     * 
     * @param sortCriteria
     * @return
    ...
    public String translateSortCriteria(final String sortCriteria) {
        ...
        for (final String sort : sorts) {
            if (didlFieldSortMap.containsKey(sort)) {
                ...
                logger.info("requested sort-criteria was not transformed : "
                        + sort);
            }
        }
        logger.info("requested sort-criteria " + sortCriteria
                + " was transformed to " + result.toString());
