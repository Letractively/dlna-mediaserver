...
//	private final static Logger logger = LoggerFactory.getLogger(StorageService.class);
	...
	public DidlXmlCreator getContainerContentById(final String containerId, final int startIdx,
			final int count, final String filter, final String sort, WebappLocationBean wlb) {
		final Query q = this.manager
				.createQuery("select didl from DIDL as didl where didl.parent.id = ?1 and didl.online = ?2" + filter  + sort);
	...
	public DidlXmlCreator getSearchItems(final String objectId, final String where, final ArrayList<Object> searchParameters, final int startIdx, final int count,
			final String filter, final String sort, WebappLocationBean wlb) {
	...
	public Long getContainerContentChildCountById(final String containerId, final String filter) {
		return (Long) this.manager.createQuery("select count(didl) from DIDL as didl where didl.parent.id = ?1 and didl.online = ?2"
	...
	public DidlXmlCreator getInfoById(final String containerId, final int startIdx, final int count,
			final String filter, final String sort, WebappLocationBean wlb) {
		final Query q = this.manager.createQuery("select didl from DIDL as didl where didl.id = ?1"
				+ filter + sort);
		q.setParameter(1, containerId);
		q.setParameter(2, true);
		q.setMaxResults(count);
	...
	public List<StringKeyValuePair> getVideoFileIdsWithoutMeta() {
		final Query q = this.manager.createQuery("select new de.sosd.mediaserver.bean.StringKeyValuePair(file.id,file.path) from DIDL where online = ?6 and (passedMPlayer is null or passedMPlayer = ?5) and (" +
				"classType = ?1 or " +
				"classType = ?2 or " +
				"classType = ?3 or " +
				"classType = ?4)"
	...
	public List<StringKeyValuePair> getVideoFileIdsWithoutThumbnail() {
		final Query q = this.manager.createQuery("select new de.sosd.mediaserver.bean.StringKeyValuePair(file.id,file.path) from DIDL where online = ?7 and (generateThumbnail is null or generateThumbnail = ?6) and passedMPlayer = ?5 and (" +
				"classType = ?1 or " +
				"classType = ?2 or " +
				"classType = ?3 or " +
				"classType = ?4)"
	...
	public List<String> getImageFileIdsWithoutMeta() {
		final Query q = this.manager.createQuery("select file.id from DIDL where online = ?7 and (generateThumbnail is null or generateThumbnail = ?6) and (" +
					"classType = ?1 or " +
					"classType = ?2" +
				")"
				);
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public void setOnline(String scanFolderId, boolean value) {
		manager.createQuery("update DIDL set online = ?2 where folder = ?1").setParameter(1, scanFolderId).setParameter(2, value).executeUpdate();
	}
