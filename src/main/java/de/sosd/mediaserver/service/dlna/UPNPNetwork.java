...
public class UPNPNetwork {
    ...
    public final static byte[]       UPNP_HOST_ADDRESS                   = {
    ...
    private void sendStatus(final String state) {
        ...
        try {
            ...
            for (final NetworkDeviceBean ndb : this.network.getActiveDevices()) {

                // sendMessage(ssdpSocket,
                // buildDiscoverMsg(URN_SCHEMAS_MEDIA_SERVER));
                // sendMessage(ssdpSocket, buildMsg(UPNP_ROOTDEVICE, ALIVE,
                // true));
                // sendMessage(ssdpSocket, buildMsg(null, ALIVE, true));
                sendMessage(
                        ssdpSocket,
                        buildNotifyMsg(ndb.getIpAddress(),
                                URN_SCHEMAS_MEDIA_SERVER, state));
                // sendMessage(ssdpSocket,
                // buildMsg(URN_SCHEMAS_CONTENT_DIRECTORY, ALIVE, true));
                // sendMessage(ssdpSocket,
                // buildMsg(URN_SCHEMAS_CONNECTION_MANAGER, ALIVE, true));
                // sendMessage(ssdpSocket, buildMsg(
                // URN_X_MS_MEDIA_RECEIVER_REGISTRAR_1, ALIVE,true));
    ...
    private void sendReply(final InetAddress remoteAddress, final int port,
            final String msg)
            throws IOException {
    ...
    private void sendMessage(final MulticastSocket ssdpSocket, final String msg)
        ...
        logger.trace("send message [" + this.upnpGroup + ":" + UPNP_PORT
                + "] -> " + msg);
    }

    private String buildNotifyMsg(final String address, String nt,
        ...
        sb.append(CRLF);
        sb.append("Location: "
                + this.cfg.getWebappLocation(address).getUrlString()
                + DLNAController.getDescriptionRef());
        sb.append(CRLF);
        sb.append("USN: " + usn);
        sb.append(CRLF);
    ...
    private String buildDiscoverMsg(final String address, final String st) {
                ...
                try {
                    ...
                    if (text.contains("M-SEARCH")) {
                        ...
                        for (final String schema : new String[] {
                                URN_SCHEMAS_CONTENT_DIRECTORY,
                                URN_SCHEMAS_MEDIA_SERVER, UPNP_ROOTDEVICE,
                                URN_X_MS_MEDIA_RECEIVER_REGISTRAR_1,
                                UPNPNetwork.this.cfg.getUSN() }) {
                            if (text.contains(schema)) {
                                sendReply(
                                        data.getAddress(),
                                        data.getPort(),
                                        buildDiscoverMsg(data.getAddress()
                                                .getHostAddress(), schema));
                                answered = true;
                            }
                        }
                        if (!answered) {
