...
public class ContentDirectoryService {
    ...
    private final static Log         logger = LogFactory
                                                    .getLog(ContentDirectoryService.class);
    ...
    public int getSystemUpdateId() {
        return this.systemDao.getSystemUpdateId(this.cfg.getUSN());
    }

    public void browseMetadata(final Browse request,
            final BrowseResponse response, final DeviceByRequestHeader device,
            final WebappLocationBean wlb) throws JAXBException, IOException {
        ...
        final String sort = this.helper.translateSortCriteria(request
                .getSortCriteria());
        ...
        if (logger.isTraceEnabled()) {
            traceRequestAndResponse(request.getBrowseFlag().value(),
                    request.getObjectID(), request.getFilter(),
                    request.getStartingIndex(), request.getRequestedCount(),
                    request.getSortCriteria(), response.getResult(),
                    response.getNumberReturned(), response.getTotalMatches(),
                    response.getUpdateID());
        } else {
            logger.debug("BrowseMetadata [" + request.getObjectID() + ","
                    + request.getStartingIndex() + ","
                    + request.getRequestedCount() + "] -> ["
                    + response.getNumberReturned() + ","
                    + response.getTotalMatches() + "]");
        }
    }

    private void traceRequestAndResponse(final String requestType,
            final String objectID, final String filter,
            final Integer startingIndex, final Integer requestedCount,
            final String sortCriteria,
            final String result, final int numberReturned,
            final int totalMatches, final int updateID) {
        logger.trace("trace request :\n(" + requestType + " " + objectID
                + " filter=" + filter + " sort by=" + sortCriteria + ") "
                + startingIndex + "," + requestedCount + "\n"
                + "(found=" + numberReturned + " of matches=" + totalMatches
                + ", updateId=" + updateID + ")\n"
                + result
                );
    }
    ...
    private void createResponse(final ContentResponse response,
            final DidlXmlCreator didlLite) throws JAXBException, IOException {
    ...
    public void browseDirectChildren(final Browse request,
            final BrowseResponse response, final DeviceByRequestHeader device,
            final WebappLocationBean wlb) throws JAXBException, IOException {
        final String objectId = getLocalObjectId(request.getObjectID());
        final String filter = this.helper.translateFilter(request.getFilter());
        ...
        if (logger.isTraceEnabled()) {
            ...
            logger.info("BrowseDirectChildren [" + request.getObjectID() + ","
                    + request.getStartingIndex() + ","
                    + request.getRequestedCount() + "] -> ["
                    + response.getNumberReturned() + ","
                    + response.getTotalMatches() + "] ");
    ...
    public void search(final Search request, final SearchResponse response,
        ...
        logger.info("Search [" + request.getContainerID() + ","
                + request.getStartingIndex() + ","
                + request.getRequestedCount() + ","
        ...
        final String where = this.helper.translateSearchCriteria(
                request.getSearchCriteria(), searchParameters);
        ...
        createResponse(response, didl);
        if (logger.isTraceEnabled()) {
            traceRequestAndResponse("Search", request.getContainerID(),
                    request.getFilter(), request.getStartingIndex(),
                    request.getRequestedCount(), request.getSortCriteria(),
                    response.getResult(), response.getNumberReturned(),
                    response.getTotalMatches(), response.getUpdateID());
        } else {
            logger.info("Search [" + request.getContainerID() + ","
                    + request.getStartingIndex() + ","
                    + request.getRequestedCount() + ","
