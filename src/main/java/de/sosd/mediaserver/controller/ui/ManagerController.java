...
public class ManagerController {
    ...
    public ModelAndView show(final HttpServletRequest request) {
        ...
        try {
            final InetAddress[] allByName = InetAddress.getAllByName(request
                    .getServerName());

            for (final InetAddress ia : allByName) {
                if (!ia.isLoopbackAddress()) {
                    this.cfg.updateWebappConfiguration(request.getScheme(),
                            ia.getHostName(), request.getServerPort(),
    ...
    public RedirectView updateServerSettings(
        ...
        this.backend.updateServerSettings(name, networkInterface, previews,
                mplayer, mencoder);
    ...
    public RedirectView removeScanfolder(
            @RequestParam("folder") final String folderId) {
    ...
    public RedirectView addScanfolder(
            @RequestParam("folder") final String folderPath) {
    ...
    public RedirectView updateScanfolder(
            @RequestParam("folder") final String folderId,
            @RequestParam("scan_interval") final String scanInterval) {
