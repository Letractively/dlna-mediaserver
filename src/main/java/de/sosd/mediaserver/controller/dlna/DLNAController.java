...
public class DLNAController {
    ...
    private static final String      SERVER_DESCRIPTION_REF = DLNA_BASE
                                                                    + "/Mediaserver.xml";
    ...
    private final static String      CAPABILITIES           = "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
                                                                    ...
                                                                    + CRLF
                                                                    +
                                                                    // "	<URLBase>$SERVER_BASE_URL</URLBase>"
                                                                    // + CRLF +
                                                                    "	<device>"
                                                                    + CRLF
                                                                    + "		<UDN>uuid:$UDN</UDN>"
                                                                    ...
                                                                    + CRLF
                                                                    +
                                                                    "				<SCPDURL>/mediaserver/resources/X_MS_MediaReceiverRegistrar.xml</SCPDURL>"
                                                                    + CRLF
                                                                    +
                                                                    "			</service>"
                                                                    + CRLF
    ...
    public DLNAController() throws JAXBException {
        this.context = JAXBContext.newInstance(
                ...
                GetSearchCapabilities.class,
                GetSearchCapabilitiesResponse.class);
    }

    private String getCapablities(final WebappLocationBean wlb) {
    ...
    @RequestMapping(value = "Mediaserver.xml", method = { RequestMethod.GET,
            RequestMethod.POST })
    public void showDescription(final HttpServletRequest request,
        ...
        try {
            ...
            response.addHeader("Content-Length", ""
                    + capablities.getBytes().length);
            response.addHeader("Date",
                    sdf.format(new Date(System.currentTimeMillis())) + " GMT");
    ...
    private WebappLocationBean getWebappLocation(
            final HttpServletRequest request) {
        return this.cfg.getWebappLocation(request.getScheme(),
    ...
    private final static String[] rangeHeaders = { "RANGE", "range",
                                               "timeseekrange.dlna.org" };

    @RequestMapping(value = "content/{type}/{uuid}.{extension}", method = {
    ...
    public void getMedia(
            ...
            @RequestParam(required = false, value = "width") final Integer width,
            @RequestParam(required = false, value = "height") final Integer height,
        ...
        if (type.equals("video") || type.equals("audio")) {
            String range = null;
            for (final String header : rangeHeaders) {
            ...
            resizeTheImage = width != null && height != null
                    && (type.equals("image") || type.equals("thumb"));
        }
        ...
        try {
            ...
            if (type.equals("thumb")) {
                content = this.thumbs.getFile(uuid, extension, width, height);
            } else {
            ...
            if (!content.exists()) {
                throw new FileNotFoundException("file "
                        + content.getAbsolutePath() + " is not present!");
            }
            response.setContentType(this.didlService
    ...
    private void logRequest(final String method,
    ...
    // Envelope envelope =
    // (Envelope)soap.createUnmarshaller().unmarshal(request.getInputStream());
    //
    // JAXBContext browse = JAXBContext.newInstance(BrowseRequest.class,
    // BrowseResponse.class);
    // BrowseRequest brequest =
    // (BrowseRequest)browse.createUnmarshaller().unmarshal((Node)
    ...
    // sm.setProperty("com.sun.xml.internal.bind.namespacePrefixMapper", new
    // PreferredNamespaceMapper());
    ...
    // public void contentGetSystemUpdateID(HttpServletRequest request,
    ...
    public void handleX_MS_MediaReceiverRegistrarIsAuthorized(
            final HttpServletRequest request,
    ...
    public void handleX_MS_MediaReceiverRegistrarIsValidated(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
    ...
    public void handleX_MS_MediaReceiverRegistrarRegistrationRespMsg(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
        logRequest("handleX_MS_MediaReceiverRegistrarRegistrationRespMsg",
                request);
    ...
    public void handleConnectionManagerGetCurrentConnectionInfo(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
    ...
    public void handleConnectionManagerConnectionComplete(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
    ...
    public void handleConnectionManagerPrepareForConnection(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
    ...
    public void handleConnectionManagerGetProtocolInfo(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
    ...
    public void handleConnectionManagerGetCurrentConnectionIDs(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
    ...
    public void handleContentDirectoryGetSystemUpdateID(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
        handleContentDirectory(request, response);
    ...
    public void handleContentDirectoryGetSortCapabilities(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
        handleContentDirectory(request, response);
    ...
    public void handleContentDirectoryGetSearchCapabilities(
            final HttpServletRequest request,
            final HttpServletResponse response) throws IOException,
            JAXBException {
        handleContentDirectory(request, response);
    ...
    private void handleContentDirectory(final HttpServletRequest request,
        ...
        final Unmarshaller um = this.context.createUnmarshaller();
        final Envelope envelope = (Envelope) um.unmarshal(is); // parsed.getValue();
        // System.out.println("recieved " +
        // envelope.getClass().getCanonicalName());
        ...
        for (final Object o : any) {
            try {
                        ...
                        if (o instanceof Search) {
                            ...
                            if (o instanceof GetSortCapabilities) {
                                ...
                                if (o instanceof GetSearchCapabilities) {
                                    ...
                                    logger.error("got unsupported soap-call : "
                ...
                logger.error("error while processing soap-request : "
                        + o.getClass().getCanonicalName(), t);
            }
        }
    ...
    private GetSearchCapabilitiesResponse handle(
            final GetSearchCapabilities request) {
    ...
    private SearchResponse handle(final Search request,
            final DeviceByRequestHeader device, final WebappLocationBean wlb)
    ...
    private BrowseResponse handle(final Browse request,
            final DeviceByRequestHeader device, final WebappLocationBean wlb)
            throws JAXBException, IOException {
