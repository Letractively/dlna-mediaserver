...
@Configurable
public class MPlayerThumbnailGeneratorProcess extends
        DefaultExecuteResultHandler implements ProcessKilledNotifier {

    private final static Log            logger = LogFactory
                                                       .getLog(MPlayerThumbnailGeneratorProcess.class);
    ...
    public MPlayerThumbnailGeneratorProcess(final File mplayer,
            final File previewFolder, final String fileId, final String path) {
        super();
        this.fileId = fileId;
        this.path = path;
    ...
    public MPlayerThumbnailGeneratorProcess execute() throws ExecuteException,
            IOException {
        debug();
        prepare();
    ...
    private void prepare() throws IOException {
        ...
        if (!this.tempFolder.mkdirs() && !this.tempFolder.exists()) {
            throw new IOException(
                    "can't create tempdirectory to store previews ..."
                            + this.tempFolder.getAbsolutePath());
        }
        // }
        ...
        this.seekable = didl.getSeekable() == null
                || didl.getSeekable().booleanValue();
        if (this.seekable) {
            ...
            this.arguments = new String[] { "-slave", "-frames", "10",
                    "-nocache", "-noidle", "-speed", "24.0", "-ao", "null",
                    "-ss", "120", "-vf", "screenshot", "-vo", "jpeg:outdir=." };
            this.watcher.addProcessToWatch(new String[] { "mplayer", "-slave",
                    "-frames", "10", "-nocache", "-noidle", "-speed", "24.0",
                    "-ao", "null", "-ss", "120", "-vf", "screenshot", "-vo",
                    "jpeg:outdir=.", this.path }, 60, this);
        } else {
            this.arguments = new String[] { "-slave", "-frames", "250",
                    "-nocache", "-noidle", "-speed", "24.0", "-ao", "null",
                    "-vf", "screenshot", "-vo", "jpeg:outdir=." };
            this.watcher.addProcessToWatch(new String[] { "mplayer", "-slave",
                    "-frames", "250", "-nocache", "-noidle", "-speed", "24.0",
                    "-ao", "null", "-vf", "screenshot", "-vo", "jpeg:outdir=.",
                    this.path }, 60, this);
        }
    ...
    private void update() {
        ...
        if (fileList != null) {
            ...
            logger.error("for soem reason the tempfolder "
                    + this.tempFolder.getAbsolutePath()
                    + " does not exist anymore, this is odd!");
        }

        try {
            if (screenshot.exists()) {
                final String type = "jpg";
                final File thumbnailFile = new File(this.thumbnailFolder, dd
                        .getId().concat(".").concat(type));
                if (thumbnailFile.exists()) {
                    // replace it
                    FileUtils.forceDelete(thumbnailFile);
                }
                // ThumbnailDomain thumb = resizeImageTo(160f, type,
                // thumbnailFile, screenshot);
                final ThumbnailDomain thumb = moveImageTo(type, thumbnailFile,
                        screenshot);
                ...
                if (this.out.toString().contains("Stream not seekable!")
                        && (dd.getSeekable() == null || dd.getSeekable()
                                .booleanValue())) {
                    ...
                    dd.setGenerateThumbnail(false);
                    this.didlDao.store(dd);
                    logger.error("update add thumb for [" + dd.getId()
                            + "] failed, no thumbnail created in "
                            + this.tempFolder.getAbsolutePath());
                    logger.error("errors : \n" + this.err.toString());
            ...
            try {
                ...
                logger.error("could not remove tempdirectory : "
                        + this.tempFolder.getAbsolutePath(), e);
            }
        }
    }

    private ThumbnailDomain moveImageTo(final String type,
        ...
        return new ThumbnailDomain(type, originalImage.getWidth() + "x"
    ...
    private void debug() {
        ...
        // if (lcpath.endsWith("ma4v") || lcpath.endsWith("mts") ||
        // lcpath.endsWith("mp4") )
        if (this.fileId
                .equalsIgnoreCase("88cb970a-ce52-3943-a057-5fa18b65cdf6"))
        {
