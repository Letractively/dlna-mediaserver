...
@Configurable
public class MPlayerMetaInfoReaderProcess extends DefaultExecuteResultHandler
        implements ProcessKilledNotifier {

    private final static Log            logger = LogFactory
                                                       .getLog(MPlayerMetaInfoReaderProcess.class);
    ...
    public MPlayerMetaInfoReaderProcess(final File mplayer,
            final String fileId, final String path) {
        super();
        this.fileId = fileId;
        this.path = path;
    ...
    public MPlayerMetaInfoReaderProcess execute() throws ExecuteException,
            IOException {
        prepare();
        ...
        // } else {
        // throw new
        // ExecuteException("mts will not work with mplayer -identify", 0);
        // }
        return this;
    }
    ...
    private void prepare() {
        ...
        // }
        this.arguments = new String[] { "-slave", "-ao", "null", "-vo", "null",
                "-frames", "0", "-nocache", "-noidle", "-identify", "-speed",
                "24.0" };
        this.watcher
                .addProcessToWatch(new String[] { "mplayer", "-slave", "-ao",
                        "null", "-vo", "null", "-frames", "0", "-nocache",
                        "-noidle", "-identify", "-speed", "24.0", this.path },
                        60, this);
    }
    ...
    private void update() {
        try {
            ...
            final String filtered_text = this.out.toString()
                    .replace('\r', '\n');
            ...
            /**
             * ID_VIDEO_ID=0 ID_AUDIO_ID=1 ID_CLIP_INFO_NAME0=Software
             ...
             * ID_VIDEO_FPS=23.976 ID_VIDEO_ASPECT=0.0000 ID_LENGTH=5502.62
             * ID_SEEKABLE=1 ID_CHAPTERS=0 ID_VIDEO_CODEC=ffodivx
             * ID_VIDEO_ASPECT=2.3529 ID_AUDIO_CODEC=ffmp3
             * 
             * ID_DEMUXER=audio ID_AUDIO_FORMAT=85 ID_AUDIO_BITRATE=192000
             * ID_AUDIO_RATE=44100 ID_AUDIO_NCH=0 ID_LENGTH=80.00 ID_SEEKABLE=1
             * ID_CHAPTERS=0 ID_CLIP_INFO_NAME0=Title
             * ID_CLIP_INFO_VALUE0=b-vert�re ID_CLIP_INFO_NAME1=Artist
             * ID_CLIP_INFO_VALUE1=Bela B. ID_CLIP_INFO_NAME2=Album
             * ID_CLIP_INFO_VALUE2=Bingo ID_CLIP_INFO_NAME3=Year
             * ID_CLIP_INFO_VALUE3=2006 ID_CLIP_INFO_NAME4=Comment
             * ID_CLIP_INFO_VALUE4=-] MST [- ID_CLIP_INFO_NAME5=Track
             * ID_CLIP_INFO_VALUE5=1 ID_CLIP_INFO_NAME6=Genre
             * ID_CLIP_INFO_VALUE6=Punk Rock ID_CLIP_INFO_N=7
             * */
            for (final String line : lines) {
                ...
                if (line.startsWith("ID")) {
                    ...
                    if (split.length == 2) {
                        parsedValues.put(split[0].substring(3).toLowerCase(),
                                split[1]);
                    }
                }
            ...
            if (infoNr != null) {
                ...
                for (int i = 0; i < idx; ++i) {
                    final String name = parsedValues.remove("clip_info_name"
                            + i);
                    final String value = parsedValues.remove("clip_info_value"
                            + i);

                    parsedValues.put(name.toLowerCase(), value);
                }
            }
            // duration="01:56:36.780" resolution="620x256" bitrate="107520"
            // nrAudioChannels="2" sampleFrequency="48000"
            ...
            dd.setBitrate(getBitrate(parsedValues.get("video_bitrate"),
                    parsedValues.get("audio_bitrate")));
            ...
            dd.setResolution(getResoulution(parsedValues.get("video_width"),
                    parsedValues.get("video_height")));
            ...
            final boolean notReadable = filtered_text.toLowerCase().contains(
                    "file not found")
                    || filtered_text.toLowerCase().contains("failed to open");
    ...
    private String getDuration(final String f) {
        ...
        try {
            ...
            return get2DigitString(hours) + ":" + get2DigitString(minutes)
                    + ":" + get2DigitString(seconds) + "."
                    + get3DigitString(millis);
        } catch (final NumberFormatException nfe) {
            return null;
        }
    }
